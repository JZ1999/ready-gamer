...
**Where we stand:** everything from 09-21/09-22 (below) plus two real fixes found during a full manual speedrun playtest are **committed and pushed** to `origin/feature/map5-spawn-locks` (7 commits, up through `33d2ce3`). Final Release ROM re-verified: MBC5 (0x19), 128 KB, title "READY GAMER", clean build, no debug scaffolding active. **Not merged to main, no PR opened, not flashed.**

**Second bug found and fixed (same session, during the full-playthrough speedrun test):** the "stray A glyphs" cosmetic issue below turned out to recur in two different, unrelated rooms, not tied to the map5 wall-gap edit (confirmed by asking — the room shown didn't even have that edit). Root cause: the room game's own background scroll streams tiles only near the camera, so leftover text `StateGameOver.c`/`StateWin.c` write straight into background tiles (`PRINT_BKG "PRESS A"`) can survive in the wrapped-around area of the 32x32 VRAM tilemap that hasn't been scrolled over yet, then resurface as stray glyphs once the camera later reaches that spot. **Fixed:** `fill_bkg_rect(0, 0, 32, 32, 0)` wipes the whole background tilemap at the top of `StateGame::START()`, before the room's own map loads (display is already off there, so it's timing-safe). Not yet re-tested over a long play session to confirm it's fully gone, just built+verified compiling and the fix logic — watch for recurrence.

**Also observed, not a bug:** BGB's debugger can trap "accessing inaccessible VRAM" (STAT mode 3 + LCD on) during normal play — this is the pre-existing, already-documented cosmetic VRAM-write-timing risk (see "Known minor risk" further below), just visibly caught by BGB's strict mode instead of silently corrupting one background tile like it would on real hardware / a less strict emulator. Not fixed (would need a `di`/`ei` patch inside the ZGB engine itself); user was told to just hit Run/F5 in BGB's debugger to continue.
